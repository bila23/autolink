...
public class RepuestoSolicitudDao extends Dao<Respuestoxsolicitud, Integer> {
    ...
    public List<Respuestoxsolicitud> findBySolicitudAndAplica(int idSol, String aplica) throws Exception{
        Map<String, Object> param = new HashMap();
        param.put("idsolicitud", idSol);
        param.put("aplica", aplica);
        return findWithNamedQuery("Respuestoxsolicitud.findBySolicitudAndAplica", param);        
    }
    ...
    public Respuestoxsolicitud findBySolicitudAndRep(Integer idSol, Integer idRep) throws Exception {
        Query q = em.createNamedQuery("Respuestoxsolicitud.findBySolAndRep");
        q.setParameter("idsolicitud", idSol);
        q.setParameter("idrepuesto", idRep);
        List<Respuestoxsolicitud> list = q.getResultList();
        if (list != null) {
            if (list.size() > 0) {
                return list.get(0);
            }
